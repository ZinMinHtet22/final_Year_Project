...
import { UserPlus, Mail, Lock, User, AlertCircle, UtensilsCrossed, Coins, CookingPot, Activity, Bot, Eye, EyeOff } from 'lucide-react'
...
export default function RegisterPage() {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to register account. Email may already be in use.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="auth-page-wrapper">
      <div className="auth-container">
        ...
        </div>        {/* Right Side: Form Panel */}
        <div className="auth-form-panel">
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
            <UserPlus size={40} color="var(--emerald)" style={{ marginBottom: '1rem', filter: 'drop-shadow(0 0 10px var(--emerald-glow))' }} />
            <h2 style={{ color: 'var(--text-primary)', fontSize: '1.75rem', fontWeight: 'bold', margin: '0 0 0.5rem 0', fontFamily: "'Outfit', sans-serif" }}>Create Account</h2>
            <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', margin: 0 }}>Create an account to access your curated kitchen workspace.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
              <label style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', fontWeight: '500', textAlign: 'left' }}>Full Name</label>
            ...
            {/* Password Input Alert Box */}
            <div style={{
              backgroundColor: 'rgba(239, 68, 68, 0.05)',
              border: '1px solid rgba(239, 68, 68, 0.25)',
              borderRadius: '8px',
              padding: passwordError && !googleClicked ? '0.75rem 1rem' : '0px 1rem',
              color: '#f87171',
              fontSize: '0.82rem',
              lineHeight: '1.4',
              textAlign: 'left',
              ...
              opacity: passwordError && !googleClicked ? 1 : 0,
              maxHeight: passwordError && !googleClicked ? '120px' : '0px',
              overflow: 'hidden',
              marginBottom: passwordError && !googleClicked ? '0.75rem' : '0px',
              transform: passwordError && !googleClicked ? 'translateY(0)' : 'translateY(-10px)',
              pointerEvents: passwordError && !googleClicked ? 'auto' : 'none'
            }}>
              <AlertCircle size={16} style={{ flexShrink: 0, marginTop: '2px', color: '#ef4444' }} />
              <span>Security Requirement: Your password must be at least 8 characters long and include an uppercase letter and a number.</span>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
              <label style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', fontWeight: '500', textAlign: 'left' }}>Confirm Password</label>
              <div style={{ position: 'relative' }}>
                <Lock size={16} color="#6b7280" style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)' }} />
                <input 
